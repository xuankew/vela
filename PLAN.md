...
| 项目级配置 | ~~`.vela/settings.json`，与用户全局配置分层合并~~ 🔴 **推迟到 M4**（用户在「M2-F 里做 / 推到 M4」两选一里选的后者）。理由与它给 M4 添的那一层见 §3.4「M2-F 实施修正」1 |
...
├── project/     多根工作区（✅ M2-F）、文件树、文件索引；.vela/settings.json 分层合并已推到 M4
...
> `.vela/settings.json` 分层合并**推迟到 M4**（用户在两选一里选的后者，理由见
> §3.4「M2-F 实施修正」1）。
...
| 前端 bundle（gzip） | **≤ 300KB** | ✅ **M0 219.58KB**（余量 27%）→ **M2-C 244.29KB** → **M2-D 247.56KB** → **M2-E 收尾 250.60KB** → **M2-F 252.52KB** → **M2-G 254.16KB** → **M2-H 256.69KB**（余量 14.4%）→ **M3-A 收尾 269.03KB**（余量 10.32%）→ **M3-B-1 275.26KB**（余量 8.25%）→ **M3-B-2 277.66KB**（余量 7.45%）→ **M3-B-3 278.94KB**（余量 7.02%）→ **M3-B-4 282.19KB**（余量 5.94%）→ **M3-B-5 284.01KB**（余量 5.33%）→ **M3-B-6 284.61KB**（余量 5.13%）→ **M3-C 277.37KB**（余量 **7.54%**）。M0 探针脚手架已于 2026-09-14 整体删除，从 236.92KB 回落 17.34KB；此后 M1 的编辑器功能与 M2 的文件树/全局搜索/全局替换/Goto Anything/多根工作区/文件监听/大文件只读分片一路加到 256.69KB（M2-H 时是 `index-CNF4LQvD.js` 145.45 + `dist-Cx7xBNi0.js` 108.38 + `index-B0zf3Qrw.css` 2.86；**M3-B-1 之后是 `index-CYDsHyFQ.js` 162.68 + `dist-Cx7xBNi0.js` 108.38 + `index-BhMjxJ3w.css` 4.20；M3-B-2 之后是 `index-DeVDbUsH.js` 165.08 + 后两个逐字不变；M3-B-3 之后是 `index-DE5ksiWr.js` 166.36 + 后两个仍逐字不变；M3-B-4 之后是 `index-DXtGvns0.js` 169.61 + 后两个仍逐字不变；M3-B-5 之后是 `index-BOqJdQFy.js` 171.42 + 后两个仍逐字不变；M3-B-6 之后是 `index-DniRLlC2.js` 172.02 + `dist-Cx7xBNi0.js` 108.38 + `index-B2FiruYW.css` 4.21；M3-C 之后是 `index-DLQUEx4N.js` 172.95 + `dist-uhAGXbQS.js` 100.21 + `index-B2FiruYW.css` 4.21**（🔴 **CM6 那个 modulepreload chunk 的哈希与字节自 M2-E 以来第一次变**：`dist-Cx7xBNi0.js` 108.38 → `dist-uhAGXbQS.js` 100.21，引入 `lazy()` 边界后 rolldown 重画分包图、在 entry 与 vendor 之间挪了模块，⛔ 不是 CM6 被懒加载，见 §3.5「M3-C 实施修正」7；⚠️ CSS 的**哈希**变了而字节只长 0.01KB——M3-B-5a 给文字格加了一条 `.toolbox-option input[type='text'] { width: 180px }`））。⚠️ 增量最大的一步是 M2-C-4c 的 **+3.60KB**——`App.tsx` 开始 import `src/search/*`，那一整层不再被 tree-shaking 摇掉；M2-D **+3.27KB**（替换那一排 UI + 确认单 + 一条命令）；M2-E **+3.04KB**（`src/goto/*` 五个模块 + `src/ui/virtual.ts` + 浮层 CSS 0.24KB + 两条命令）；M2-F **+1.92KB**（多根 + 最近项目，⛔ 一个新浮层都没加）；M2-G **+1.64KB**（裁决框整套复用 `.modal-*`，**CSS 一个字节都没长**，哈希与 M2-F 逐字相同）；M2-H **+2.53KB**（`src/doc/shardView.ts` + `src/doc/ShardPane.tsx` + 状态栏那一排只读格 + `App.tsx` 接线；**CSS +0.17KB，哈希自 M2-F 以来第一次变**——新增的是一整套 16 条 `.shard-*` 选择器，状态栏那几格复用既有样式）。⚠️ **M2-E 起 CM6 那个 modulepreload chunk 不再是 M0 那一个字节不差的 108.31KB，变成 108.38KB（hash 也随之换了）**——没有追查原因，0.07KB 的量级不值得为它停一次工，但「一个字节没变」这句话从此不再成立 | CM6 内核 100.21KB（modulepreload chunk，🔴 **「自 M2-E 起一个字节没动」这句话到 M3-C 断了**：`dist-Cx7xBNi0.js` 108.38 → `dist-uhAGXbQS.js` 100.21，rolldown 因 `lazy()` 边界重画分包图、在 entry 与 vendor 之间挪了模块，⛔ 不是 CM6 被懒加载，编辑器仍首屏，见 §3.5「M3-C 实施修正」7）+ 入口 172.95KB（Solid + CM6 基础扩展 + 应用代码）+ 应用 CSS 4.21KB。✅ **`cmdk-solid` / `@kobalte/core` / `@ark-ui/solid` 三个 UI 依赖一个都没进场**（M3-B-1 结案，见 §2.4「UI 组件」那一行），所以「这 14.4% 不是永久空间」那句担心没有以它预想的方式兑现——**兑掉它的是 M3 自己的功能代码**。🔴 **余量 5.13% → M3-C-1/C-2 买回到 7.54%（277.37KB）**：`md/render.ts`（41KB 源）、`md/export.ts`、`md/preview.ts`、`md/MarkdownPreview.tsx`、`commands/CommandPalette.tsx`、`tools/ToolBox.tsx` **已移出首屏**（六个懒加载 chunk，见 §3.5「M3-C 实施修正」）；⛔ **`md/paste.ts` + `ipc/asset.ts` 与 `doc/stats.ts` 仍在首屏**——M3-C-3 未做：paste 卡在「同步粘贴闸」那道约束上（`acceptsPastedImage` 必须同步回答，要移得先拆模块），stats 是纯没做（字数命令本可 `await import`）。所以「M3-C 的懒加载是把余量买回来的唯一手段」这句话**只兑现了一部分**，⛔ 不是全部买回来。⛔ **`tools/registry.ts` + `tools/builtin.ts` + `tools/json.ts` + `tools/codec.ts` + `tools/uuid.ts` + `tools/time.ts` + `tools/regex.ts` + `tools/naming.ts` + `util/base64.ts` 九个圈不进去**（工具要在启动时投影成命令），M3-B-2 那 2.40KB、M3-B-3 那 1.28KB、M3-B-4 那 3.25KB、M3-B-5 那 1.82KB 与 M3-B-6 那 0.60KB 因此都是首屏常驻的，见 §3.5「M3-B-2 实施修正」14、「M3-B-3 实施修正」11、「M3-B-4 实施修正」14、「M3-B-5 实施修正」15 与「M3-B-6 实施修正」9 |
...
> 2. ⛔ **「路径与命名可配置」没有交付，推给 M4。** 那一半依赖 `.vela/settings.json` 项目级配置，而 M2-F 时用户在「M2-F 里做 / 推到 M4」两选一里选了后者（见 §3.4「M2-F 实施修正」1）。所以现在是**写死的**：目录 = 文档所在目录下的 `assets/`（`ASSET_DIR`），文件名 = `pasted-<16 位十六进制内容哈希>.<ext>`（`NAME_PREFIX`），撞名换后缀（`MAX_SUFFIX` = 32）。⚠️ 哈希命名带来一个**好性质**：同一张图粘两次只落一个文件，第二次直接复用（`StoredImage.reused: true`），而且**不说话**——插进正文的那行链接就是全部反馈，与 M3-A-5「对齐成功了就不该说话」同一条口径。⚠️ M4 真做配置时必须回头改 `md/paste.ts` 里那句「rel 不含空格、括号或引号所以不需要 `<>` 包裹也不需要转义」——命名规则一旦可配置，那句话就不成立了。
...
| 项目级配置 | 1 | 🔴 **M2-F 推过来的**（用户在「M2-F 里做 / 推到 M4」两选一里选的后者，理由见 §3.4「M2-F 实施修正」1）：`.vela/settings.json` + 用户全局 `~/.vela/settings.json` + 内置默认，**三层合并**；「哪一层能改哪些键」要一并定下来。⚠️ 上面那张 M2-A 修正 4 里说的「类型分组排序」要是哪天有人要，开关就落在这一层 |
