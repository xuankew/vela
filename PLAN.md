...
- 内置：~~Vela Dark、Vela Light、High Contrast，+ 3 套流行配色移植~~ ✅ **M4-C 阶段1 交付亮 / 暗 2 套**（Tokyo Night 暗 / Tokyo Night Day 亮）；⛔ **High Contrast 与 3 套流行配色移植推迟到阶段2**（**注意核对配色方案各自的 License**，多为 MIT，但需逐个确认，见 §2.10 / R7）。
...
- 用户自定义主题：放一个 JSON 文件到 `~/.vela/themes/` 即可加载（这也是将来插件的第一个能力出口）。⛔ **推迟到阶段2**；用户已裁定格式 = **JSON token 映射**（只认那 11 个颜色 token + `colorScheme`，可校验、⛔ 不能注入任意 CSS）。
...
✅ **M4-C 已交付（2026-09-21，主题系统 v1 阶段1）**：颜色从写死的暗色改成**可切换的 token 架构**——11 个 `--vela-*` 颜色变量 + `color-scheme` 拆成「暗色默认 `:root` + 亮色 `:root[data-theme='light']`」两块，`<html data-theme>` 一分派即生效；CM6 的 `EditorView.darkTheme` facet 从写死 `true` 改成一个**共享 Compartment**（`darkSlot`），随主题一起翻。内置**亮 / 暗**两套 + **跟随系统**（matchMedia 订阅），主题选择器收进既有「外观」浮层（多一个 select，⛔ 没加工具栏按钮），`theme` 是写穿 `~/.vela/settings.json` 的**第六个键**。⚠️ **默认 `dark` = 升级零像素变化**（老用户本来就在写死的暗色上）。⛔ **阶段2 推迟**：3 套流行配色移植（逐个核 License）+ `~/.vela/themes/` 自定义主题加载——用户裁定自定义格式 = **JSON token 映射**（只认那 11 个颜色 token + colorScheme，可校验、⛔ 不能注入任意 CSS），见 §3.6「M4-C 实施修正」。
...
| 工作项 | 人日 | 要点 |
|---|---|---|
| 主题系统 | 1.5 | ⚠️ **M4-C 阶段1 兑现（2026-09-21）**：✅ CSS Variables 颜色架构（11 个 `--vela-*` token + `color-scheme`，`<html data-theme>` 分派）、✅ 内置**亮 / 暗**两套、✅ 跟随系统（matchMedia 订阅）、✅ CM6 `darkTheme` facet 改成可翻的共享 Compartment、✅ 选择器收进「外观」浮层、✅ `theme` 键写穿配置（第六个键）。⛔ **两处推迟到阶段2**：① ~~3 套流行配色移植（逐个核 License）~~；② ~~用户自定义主题加载 `~/.vela/themes/`~~——用户裁定自定义格式 = **JSON token 映射**（只认 11 个颜色 token + colorScheme，可校验、不能注入任意 CSS）。⚠️ 原行写「内置 3 套基础主题」，阶段1 只做了**亮/暗 2 套**（第三套留给阶段2 的流行配色）。详见下面「M4-C 实施修正」 |
