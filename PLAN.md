...
**结论：🟢 继续按 Tauri 2 路线走，M0 闸门有条件通过** —— 5 项通过（#3 #5 #6 #7 #8）、**#4 机制 ✅ 但数值 ❌（首屏 2.219MB，超 2MiB 预算 11%）**、#1 客观半通过、#2 未测。M1 开工前必须补齐的项见报告 §5（其中只有 #2 中文 IME 需要人来测）。上表的「当前状态」列与本节是同一批数据的就地批注版，**引用结论时以报告为准**。
> 📌 **#4 不阻塞路线**：超标 11% 的根因是「按码点区块切分」让一屏字符散落到 25~36 个分片，修法是按字频重排分片（见上表 #4 行），属于 M1 的构建脚本改动，不触及架构。
...
**剩余工作**（截至 2026-09-14，#3 #4 #5 #6 #7 #8 已结项，其中 #4 是「机制通过 / 数值超标」）：
- **#2 中文 IME 是唯一结构性不可自动化的一项**，必须在真实 WKWebView 窗口里人工输入判定。程序化插入文本会绕过 `compositionstart/update/end`——而那正是被测路径；合成键盘事件也驱动不了 IME 候选窗。面板里已备原生 textarea 对照组，用于区分是 CM6 的问题还是 WKWebView 的问题。
...
- **#4 已测完，判为「机制 ✅ / 数值 ❌ 超预算 11%」**：首屏 2.219MB vs 2MiB 预算。根因是分片按码位区块切，修法（按字频重排）转 M1，属构建脚本改动，不阻塞路线；**dPR 不影响本项，无需 Retina 复测**。详见上表 #4 行与 M0-REPORT §2。
