...
- **CSS Variables 驱动**：主题 = 一组变量值，切换零重启、无重新编译。✅ **M4-C 阶段1 已落地**：11 个 `--vela-*` 颜色 token + `color-scheme`，`<html data-theme>` 分派（见 §3.6「M4-C 实施修正」）。
- 内置：~~Vela Dark、Vela Light、High Contrast，+ 3 套流行配色移植~~ ✅ **M4-C 阶段1 交付亮 / 暗 2 套**（Tokyo Night 暗 / Tokyo Night Day 亮）；✅ **M4-C-2 交付 3 套流行配色**（Dracula / Nord / Solarized，均为 MIT License，见 §3.6「M4-C-2 实施修正」）；⛔ **High Contrast 推迟到后续**。
- 跟随系统深浅色（macOS `prefers-color-scheme`）。✅ **M4-C 阶段1 已落地**：`theme='system'` 时挂 matchMedia 订阅，选死亮/暗则退订（OS 切换不盖用户明确选择）。
- 编辑器语法配色与 UI 配色分离，便于用户只改一部分。⚠️ **M4-C 阶段1 未做这层分离**：颜色 token 是一份，CM6 与 UI 共用；语法配色的独立微调留给后续。
- 用户自定义主题：放一个 JSON 文件到 `~/.vela/themes/` 即可加载（这也是将来插件的第一个能力出口）。⛔ **推迟到后续**；用户已裁定格式 = **JSON token 映射**（只认那 11 个颜色 token + `colorScheme`，可校验、⛔ 不能注入任意 CSS）。
...
字体切换：`--vela-font-editor` / `--vela-font-ui` 两个 CSS Variable，由 loader 在注入时一并覆写，切换即生效、无需重启。~~M4 的主题系统会接管这些变量。~~ ✅ **M4-C 已接管颜色那一半**：11 个 `--vela-*` 颜色 token + `color-scheme` 现在由 `<html data-theme>` 分派（见下），字体那两个变量仍由 loader 覆写、与主题正交。

✅ **M4-B 已交付（2026-09-21）**：字体切换 UI 从工具栏三个并排 select 收进一块**非模态**「外观」浮层（`settings/AppearancePopover.tsx`），并加了**行高 / 字间距**两档——新增 `--vela-line-height` / `--vela-letter-spacing` 两个 CSS Variable，与字号/字体一样由前端 store 写穿到 `~/.vela/settings.json`（五个键，见 §3.6「M4-B 实施修正」）。⚠️ **刻意维持「正文/UI 合一区 + 代码区」两区，没有拆成三区独立字体**；⛔ **按需下载额外字体推迟**（分片懒加载 M1/D7 早已落地，本轮不涉及）。

✅ **M4-C-2 已交付（2026-09-22，主题系统 v1 阶段2）**：在 M4-C 阶段1 的亮/暗 + 跟随系统基础上，新增**三套流行配色**——Dracula（紫调赛博朋克风）、Nord（冷灰北欧极简）、Solarized（暖黄低对比护眼），均为 MIT License，逐个确认过合规性。ThemeId 从 `['light', 'dark', 'system']` 扩到 `[..., 'dracula', 'nord', 'solarized']`，resolveTheme 把这三者都映射成 `'dark'`（全是暗色主题），CM6 darkTheme facet 保持开启。CSS 里加了三块 `:root[data-theme='*']` token 集，每块约 0.08KB，合计 +0.25KB；entry +0.05KB 是 theme.ts 里新增的三个 ID 与映射逻辑。⛔ **High Contrast 与自定义主题加载仍推迟**，见 §3.6「M4-C-2 实施修正」。
...
| 前端 bundle（gzip） | **≤ 300KB** | ✅ **M0 219.58KB**（余量 27%）→ **M2-C 244.29KB** → **M2-D 247.56KB** → **M2-E 收尾 250.60KB** → **M2-F 252.52KB** → **M2-G 254.16KB** → **M2-H 256.69KB**（余量 14.4%）→ **M3-A 收尾 269.03KB**（余量 10.32%）→ **M3-B-1 275.26KB**（余量 8.25%）→ **M3-B-2 277.66KB**（余量 7.45%）→ **M3-B-3 278.94KB**（余量 7.02%）→ **M3-B-4 282.19KB**（余量 5.94%）→ **M3-B-5 284.01KB**（余量 5.33%）→ **M3-B-6 284.61KB**（余量 5.13%）→ **M3-C 277.37KB**（余量 **7.54%**）→ **M4-A 277.84KB**（余量 **7.39%**）→ **M4-B 279.08KB**（余量 **6.97%**）→ **M4-C 279.78KB**（余量 **6.74%**）→ **M4-C-2 280.08KB**（余量 **6.64%**）。M0 探针脚手架已于 2026-09-14 整体删除，从 236.92KB 回落 17.34KB；此后 M1 的编辑器功能与 M2 的文件树/全局搜索/全局替换/Goto Anything/多根工作区/文件监听/大文件只读分片一路加到 256.69KB（M2-H 时是 `index-CNF4LQvD.js` 145.45 + `dist-Cx7xBNi0.js` 108.38 + `index-B0zf3Qrw.css` 2.86；**M3-B-1 之后是 `index-CYDsHyFQ.js` 162.68 + `dist-Cx7xBNi0.js` 108.38 + `index-BhMjxJ3w.css` 4.20；M3-B-2 之后是 `index-DeVDbUsH.js` 165.08 + 后两个逐字不变；M3-B-3 之后是 `index-DE5ksiWr.js` 166.36 + 后两个仍逐字不变；M3-B-4 之后是 `index-DXtGvns0.js` 169.61 + 后两个仍逐字不变；M3-B-5 之后是 `index-BOqJdQFy.js` 171.42 + 后两个仍逐字不变；M3-B-6 之后是 `index-DniRLlC2.js` 172.02 + `dist-Cx7xBNi0.js` 108.38 + `index-B2FiruYW.css` 4.21；M3-C 之后是 `index-DLQUEx4N.js` 172.95 + `dist-uhAGXbQS.js` 100.21 + `index-B2FiruYW.css` 4.21；M4-A 之后是 `index-CXlrqgld.js` 173.42 + 后两个逐字不变；M4-B 之后是 `index-K-zxq9pt.js` 174.45 + `dist-uhAGXbQS.js` 100.21（逐字不变）+ `index-D1UC1zWI.css` 4.42；M4-C 之后是 `index-DcC0Ltq2.js` 175.05 + `dist-uhAGXbQS.js` 100.21（逐字不变）+ `index-Ck8q8qUq.css` 4.52；M4-C-2 之后是 `index-2CsICmfi.js` 175.10 + `dist-uhAGXbQS.js` 100.21（逐字不变）+ `index-CsjP9OTc.css` 4.77**（🔴 **CM6 那个 modulepreload chunk 的哈希与字节自 M2-E 以来第一次变**：`dist-Cx7xBNi0.js` 108.38 → `dist-uhAGXbQS.js` 100.21，引入 `lazy()` 边界后 rolldown 重画分包图、在 entry 与 vendor 之间挪了模块，⛔ 不是 CM6 被懒加载，见 §3.5「M3-C 实施修正」7；⚠️ CSS 的**哈希**变了而字节只长 0.01KB——M3-B-5a 给文字格加了一条 `.toolbox-option input[type='text'] { width: 180px }`））。⚠️ 增量最大的一步是 M2-C-4c 的 **+3.60KB**——`App.tsx` 开始 import `src/search/*`，那一整层不再被 tree-shaking 摇掉；M2-D **+3.27KB**（替换那一排 UI + 确认单 + 一条命令）；M2-E **+3.04KB**（`src/goto/*` 五个模块 + `src/ui/virtual.ts` + 浮层 CSS 0.24KB + 两条命令）；M2-F **+1.92KB**（多根 + 最近项目，⛔ 一个新浮层都没加）；M2-G **+1.64KB**（裁决框整套复用 `.modal-*`，**CSS 一个字节都没长**，哈希与 M2-F 逐字相同）；M2-H **+2.53KB**（`src/doc/shardView.ts` + `src/doc/ShardPane.tsx` + 状态栏那一排只读格 + `App.tsx` 接线；**CSS +0.17KB，哈希自 M2-F 以来第一次变**——新增的是一整套 16 条 `.shard-*` 选择器，状态栏那几格复用既有样式）；M4-A **+0.47KB**（`settings/store.ts` + `ipc/settings.ts` + `App.tsx` 字体信号接线，全落在 entry；CSS 与 modulepreload chunk 一个字节没动，也没加任何新懒加载边界——`settingsNotice` 复用既有 `.notice warning`）；M4-B **+1.24KB**（`settings/AppearancePopover.tsx` + store 的行高/字间距两档 + `App.tsx` 工具栏字体组换成一个外观按钮，entry +1.03KB；CSS +0.21KB 是一整套 `.appearance-*` 选择器，哈希 `B2FiruYW` → `D1UC1zWI`；modulepreload chunk 逐字不变，⛔ 没加新懒加载边界）；M4-C **+0.70KB**（`settings/theme.ts` 主题注册表 + store 的 `theme` 键与 matchMedia 订阅 + `AppearancePopover` 多一个主题 select + `App.tsx` 的 `applyDark` 接线，entry +0.60KB；CSS +0.10KB 是把颜色 token 从单个 `:root` 拆成「暗色默认 `:root` + 亮色 `:root[data-theme='light']`」两块并各带一条 `color-scheme`，哈希 `D1UC1zWI` → `Ck8q8qUq`；modulepreload chunk 逐字不变，⛔ 没加新懒加载边界）。⚠️ **M2-E 起 CM6 那个 modulepreload chunk 不再是 M0 那一个字节不差的 108.31KB，变成 108.38KB（hash 也随之换了）**——没有追查原因，0.07KB 的量级不值得为它停一次工，但「一个字节没变」这句话从此不再成立 | CM6 内核 100.21KB（modulepreload chunk，🔴 **「自 M2-E 起一个字节没动」这句话到 M3-C 断了**：`dist-Cx7xBNi0.js` 108.38 → `dist-uhAGXbQS.js` 100.21，rolldown 因 `lazy()` 边界重画分包图、在 entry 与 vendor 之间挪了模块，⛔ 不是 CM6 被懒加载，编辑器仍首屏，见 §3.5「M3-C 实施修正」7）+ 入口 175.10KB（Solid + CM6 基础扩展 + 应用代码；M4-C-2，M4-C 时是 175.05）+ 应用 CSS 4.77KB。✅ **`cmdk-solid` / `@kobalte/core` / `@ark-ui/solid` 三个 UI 依赖一个都没进场**（M3-B-1 结案，见 §2.4「UI 组件」那一行），所以「这 14.4% 不是永久空间」那句担心没有以它预想的方式兑现——**兑掉它的是 M3 自己的功能代码**。🔴 **余量 5.13% → M3-C-1/C-2 买回到 7.54%（277.37KB）**：`md/render.ts`（41KB 源）、`md/export.ts`、`md/preview.ts`、`md/MarkdownPreview.tsx`、`commands/CommandPalette.tsx`、`tools/ToolBox.tsx` **已移出首屏**（六个懒加载 chunk，见 §3.5「M3-C 实施修正」）；⛔ **`md/paste.ts` + `ipc/asset.ts` 与 `doc/stats.ts` 仍在首屏**——M3-C-3 未做：paste 卡在「同步粘贴闸」那道约束上（`acceptsPastedImage` 必须同步回答，要移得先拆模块），stats 是纯没做（字数命令本可 `await import`）。所以「M3-C 的懒加载是把余量买回来的唯一手段」这句话**只兑现了一部分**，⛔ 不是全部买回来。⛔ **`tools/registry.ts` + `tools/builtin.ts` + `tools/json.ts` + `tools/codec.ts` + `tools/uuid.ts` + `tools/time.ts` + `tools/regex.ts` + `tools/naming.ts` + `util/base64.ts` 九个圈不进去**（工具要在启动时投影成命令），M3-B-2 那 2.40KB、M3-B-3 那 1.28KB、M3-B-4 那 3.25KB、M3-B-5 那 1.82KB 与 M3-B-6 那 0.60KB 因此都是首屏常驻的，见 §3.5「M3-B-2 实施修正」14、「M3-B-3 实施修正」11、「M3-B-4 实施修正」14、「M3-B-5 实施修正」15 与「M3-B-6 实施修正」9。⚠️ **M4-A 只 +0.47KB、余量 7.54% → 7.39%**：配置层是纯逻辑（store + ipc 镜像 + 三个信号接线），⛔ 没加新懒加载边界、没加一行 CSS、managed state 也没涨（无状态，现读现合并），所以这一步几乎没动预算——详见 §3.6「M4-A 实施修正」9。⚠️ **M4-B +1.24KB、余量 7.39% → 6.97%**：外观浮层把工具栏三个字体 select 收进一块**非模态**浮层、又加了行高/字间距两档，entry +1.03KB、CSS +0.21KB（一整套 `.appearance-*`），modulepreload chunk 一个字节没动、⛔ 没加新懒加载边界、managed state 仍 4——详见 §3.6「M4-B 实施修正」9。⚠️ **M4-C +0.70KB、余量 6.97% → 6.74%**：主题系统 v1 阶段1 加的是 `settings/theme.ts` 注册表 + store 的 `theme` 键与 matchMedia 订阅 + 外观浮层多一个主题 select + `App.tsx` 的 `applyDark` 接线（entry +0.60KB），CSS +0.10KB 是把颜色 token 从单块 `:root` 拆成「暗色默认 + 亮色 `[data-theme='light']`」两块（哈希 `D1UC1zWI` → `Ck8q8qUq`）；modulepreload chunk 逐字不变、⛔ 没加新懒加载边界、managed state 仍 4（`theme` 是第七个键但仍属那份无状态的偏好档）——详见 §3.6「M4-C 实施修正」。⚠️ **M4-C-2 +0.30KB、余量 6.74% → 6.64%**：三套流行配色（Dracula / Nord / Solarized，均为 MIT License）的 CSS token 块各约 0.08KB，合计 +0.25KB；entry +0.05KB 是 theme.ts 里新增的三个 ID 与 resolveTheme 映射逻辑；modulepreload chunk 逐字不变、⛔ 没加新懒加载边界、managed state 仍 4 |
...
| 工作项 | 人日 | 要点 |
|---|---|---|
| 主题系统 | 1.5 | ⚠️ **M4-C 阶段1 兑现（2026-09-21）**：✅ CSS Variables 颜色架构（11 个 `--vela-*` token + `color-scheme`，`<html data-theme>` 分派）、✅ 内置**亮 / 暗**两套、✅ 跟随系统（matchMedia 订阅）、✅ CM6 `darkTheme` facet 改成可翻的共享 Compartment、✅ 选择器收进「外观」浮层、✅ `theme` 键写穿配置（第六个键）。⛔ **一处推迟到阶段2**：~~用户自定义主题加载 `~/.vela/themes/`~~——用户裁定自定义格式 = **JSON token 映射**（只认 11 个颜色 token + colorScheme，可校验、不能注入任意 CSS）。✅ **M4-C-2 完成（2026-09-22）**：三套流行配色移植（Dracula / Nord / Solarized，均为 MIT License），详见下面「M4-C 实施修正」。⚠️ 原行写「内置 3 套基础主题」，阶段1 只做了**亮/暗 2 套**；阶段2 补了 Dracula/Nord/Solarized 三套。详见下面「M4-C 实施修正」 |
| 字体管线产品化 | 2 | ⚠️ **M4-B 部分兑现（2026-09-21）**：✅ 字体切换 UI（收进一块非模态「外观」浮层）、✅ 字号/行高/字间距设置（五个键写穿 `~/.vela/settings.json`）已交付；分片懒加载 M1/D7 早已落地。⛔ **三处与原行不同**：① ~~编辑器/UI/正文三区独立~~ **维持「正文/UI 合一 + 代码区」两区**（用户裁定，理由见「M4-B 实施修正」2）；② ~~按需下载额外字体~~ **推迟**（用户裁定，见「M4-B 实施修正」3）；③ 字体切换 UI 不是工具栏并排 select 而是一块浮层。详见下面「M4-B 实施修正」 |
...
> **M4-B 实施修正**（字体管线产品化：外观浮层 + 行高/字间距，2026-09-21）
>
> 1. 🔴 **用户裁定三条，逐条落地。** ① **排版设置收进一块「外观」浮层**：工具栏原来并排三个字体 select（字号/正文/代码）换成**一个**「外观」按钮，点开是一块浮层，里头装字号、行高、字间距、正文字体、代码字体五项 + 一个「恢复默认」。② **维持「正文/UI 合一区 + 代码区」两区**，⛔ **不拆成三区独立字体**——本轮只**新增**行高/字间距两档，字体分区一个没动（理由见第 2 条）。③ **按需下载额外字体推迟**（分片懒加载 M1/D7 早已落地，本轮不涉及）。
> 2. 🔴 **浮层是「非模态锚定下拉」，不是 ToolBox 那种模态遮罩——因为调行高/字间距必须能同时看见编辑器实时变。** 模态遮罩会压暗并挡住正文，而行高/字间距是**所见即所得**的排版项，盖住正文等于让用户盲调。于是：click-outside 用挂在 `document` 上的 `pointerdown` 监听（`rootEl.contains` 判内外），⛔ **不是一层透明遮罩**（那会吃掉编辑器上的点击）；监听器**延迟一拍**（`setTimeout 0`）再挂，否则「点开浮层那一下」会被立刻判成 outside。Esc 在浮层子树的 `onKeyDown` 里处理（`close` + `preventDefault`），⛔ **不注册成命令**——命令键绑在 window 的 capture 阶段先于 bubble 跑，而 `builtins.ts` 那条「绑 Escape 的命令一律不注册」保证了这里不会与谁抢键。
> 3. 🔴 **字间距默认 `0` 落成 CSS `normal`，不是 `0em`。** `applyLetterSpacingVar(n)` 在 `n === 0` 时写 `normal`、否则写 `${n}em`（模板串）。理由：`0em` 与 `normal` 在多数字体上视觉相同，但 `normal` 是浏览器的字间距初值，**保证默认档下一个字节都没改排版**（与「首屏零视觉变化」这条对齐）；浮层里的读数也把 `0` 显示成「正常」而不是「0em」。
> 4. 🔴 **浮点归一化是 load-bearing 的，不是美化。** 步进器累加会出尾巴（`1.75 + 0.05 = 1.8000000000000003`），`sanitizeStepper` 一律 `Math.round(n * 100) / 100` 收两位。不收的三重代价：① 读数显示成一串尾巴；② 尾巴漂移进 `settings.json`；③ **写队列指纹假阴性**——`lastSent` 比的是值，`1.8` 与 `1.8000000000000003` 不相等，于是每按一次都触发一次本可跳过的原子写。`store.test.ts` 专门钉了 `1.75 → 1.8 → 1.9` 这条链。
> 5. 🔴 **`f64` 进 serde 后，三个结构体只能 derive `PartialEq`、⛔ 不能 `Eq`。** `Settings` / `SettingsLayer` / `LoadedSettings` 含 `f64` 字段后 `Eq` 编译不过（`f64` 不是 `Eq`）。跨语言那笔账：Rust 把 `0.0` 序列化成 `0.0`、`2.0` 成 `2.0`，JS `JSON.stringify(0)` → `0`——**同一个 JSON 数，两边 parse 出来 `===`**（与 session 的 `scrollTop` 同一先例）。于是前端黄金字面量写 `letterSpacing:0`、Rust 黄金写 `0.0`，`wire_contract.rs` 与 `ipc/settings.test.ts` 各钉一条「`0.0` 与 `0` 读回来是同一个数」，漂了当场红。
> 6. ⚠️ **两边各写一份的契约常量又多了两个**：`DEFAULT_LINE_HEIGHT`(1.75) / `DEFAULT_LETTER_SPACING`(0.0) 在 Rust `settings/mod.rs` 与前端 `store.ts` 各存一份，由 `wire_contract.rs`、`ipc/settings.test.ts`、`settings/store.test.ts` 各钉（与 M4-A 那三个字体常量同一套做法，没有代码生成）。🔴 **Rust 侧刻意不夹范围**：`line_height` / `letter_spacing` 是裸 `f64`，合法区间（行高 1.0~3.0、字间距 -0.05~0.5）是 **UI 概念**，夹一次就够、只在前端 store 夹（与 `fontSize` / 字体 ID 同一分工，见 M4-A 修正 4）。
> 7. 🔴 **安全姿势一个字节没动。** `save_settings` 仍**不收任何路径参数**，落点 `~/.vela/settings.json` 由 Rust `home_dir(app)` 算；本轮**没有新增任何收前端路径的命令**（`load_settings` 的 `roots[0]` 是 M4-A 加的，M4-B 一个命令都没加）。`commands.rs` 那张「谁能碰到磁盘」的表仍 **17 行**，managed state 仍 **4**，事件名仍 **8**。
> 8. ⚠️ **CM6 光标坐标是一笔真机债（jsdom 钉不住）。** `fontTheme` / `codeDocFontTheme` 的 `&` 都加了 `letterSpacing: var(--vela-letter-spacing)`，而**非 `normal` 字间距下 CM6 的光标坐标测量**在 jsdom 里没有布局、验不了（`setup.ts` 里有 🔴 注释标记）。真机上要看：字间距拉大后点击定位、选区高亮、IME 候选框会不会错位——与 M0 #2 那笔 IME 债是同一类（dPR=1 下的人工判定）。
> 9. **数字**：前端 **2668 → 2697（+29）**：`ipc/settings.test.ts` 16 → **17**（+1，「`0.0` 与 `0` 同一个数」）、`settings/store.test.ts` 21 → **33**（+12，行高/字间距那一组）、`settings/AppearancePopover.test.tsx` 新增 **13**、`App.test.tsx`「字体/外观浮层接线」**+3**（行高步进器 / 字间距步进器 / 恢复默认）。Rust **428 → 430（+2）**：vela-core lib 342 → **344**（+2，「行高与字间距能存回来」+「项目层写行高与字间距也被忽略并记账」；「存下来再读回来一致」改名为「存下来的文件是干净的五个键_json」，不是净增）、`wire_contract.rs` **49 不变**（黄金值更新、条数没变）。🔴 **Tauri command 仍 23**（一个都没加），managed state 仍 4，事件名仍 8，路径接受面仍 17 行。新增依赖 **0**。**八道门禁全绿**，`stderr` 干净。<br>首屏 gzip 277.84KB → **279.08KB**（`index-K-zxq9pt.js` **174.45** + `dist-uhAGXbQS.js` 100.21 + `index-D1UC1zWI.css` **4.42**），预算 300KB，余量 7.39% → **6.97%**（**+1.24KB**：entry +1.03 是 `AppearancePopover.tsx` + store 两档 + `App.tsx` 接线；CSS +0.21 是一整套 `.appearance-*` 选择器，哈希 `B2FiruYW` → `D1UC1zWI`）。⚠️ **modulepreload chunk 一个字节没动**（`dist-uhAGXbQS.js` 100.21，哈希逐字相同）——M4-B 没引入任何新懒加载边界，外观浮层是同步 import 进 entry 的（它太小，懒加载省不下什么，反而多一次往返）。
> 10. 🔴 **真机上要看的（jsdom 钉不住，一个数都没有）**：① 非模态浮层在真 WKWebView 里的 click-outside 时序——延迟一拍挂监听够不够、点编辑器空白处会不会既关浮层又动了光标；② 行高/字间距改动是不是**实时**反映在正文与代码区（`--vela-line-height` 被 CM6 `fontTheme` 与 `.md-preview-body` 双方消费，`--vela-letter-spacing` 同理）；③ 第 8 条那笔光标坐标债（字间距拉大后点击/选区/IME 候选框定位）；④ 「恢复默认」五项一起打回内置默认后，重启是不是真的回来（写穿与 M4-A 同一条 promise 队列，但五项一起写是本轮新的）。

> **M4-C 实施修正**（主题系统 v1 阶段1：颜色 token 架构 + 亮/暗 + 跟随系统，2026-09-21）
>
> 1. 🔴 **用户裁定三条，逐条落地。** ① **v1 分两步走**：本轮（阶段1）= 主题**架构**（11 个颜色 token + `color-scheme` + CM6 `darkTheme` facet 改成可翻）+ 内置**亮/暗**两套 + **跟随系统** + 选择器收进既有「外观」浮层 + `theme` 键接进 M4-A 配置管线（持久化）；⛔ **阶段2 推迟** = 3 套流行配色移植（逐个核 License，见 §2.10 / R7）+ `~/.vela/themes/` 自定义主题加载。② **自定义主题格式 = JSON token 映射**（阶段2 才落地，但格式现在定死）：每个主题一份 JSON，**只认那 11 个颜色 token + `colorScheme`**，可校验、⛔ **不能注入任意 CSS**——这是安全边界，不是省事：允许任意 CSS 就等于允许自定义主题改写整个 UI 的行为。③ **选择器位置 = 收进「外观」浮层**（复用 M4-B 的 `AppearancePopover`，多一个主题 `<select>` 作第一行），⛔ **不加工具栏按钮**。
> 2. 🔴 **默认 `theme = "dark"` 是「升级零像素变化」的硬保证，不是随手挑的。** M4-C 之前颜色是写死的暗色（`:root` 一套暗色 token、CM6 `darkTheme` 写死 `true`）。把默认档定成 `dark`，老用户升级后**一个像素都没变**——亮色是新增的**可选项**，不是新默认。Rust `DEFAULT_THEME="dark"` 与前端 `theme.ts` 的 `DEFAULT_THEME='dark'` 各存一份，由 `wire_contract.rs`（「内置默认配置被钉住」断言 `DEFAULT_THEME=="dark"`）、`ipc/settings.test.ts`、`store.test.ts` 各钉（与 M4-A/M4-B 那五个常量同一套做法，没有代码生成）。
> 3. 🔴 **颜色走 CSS 变量、`darkTheme` facet 走 Compartment——两条独立的路，必须一起翻。** 颜色那 11 个 token + `color-scheme` 全在 CSS 里：store 把解析出的具体主题写进 `<html data-theme='light'|'dark'>`（`applyThemeAttr`），`:root[data-theme='light']` 那块 CSS 立即接管，**CM6 不需要 reconfigure**（编辑器消费的是同一批 `--vela-*` 变量）。但 CM6 的 `EditorView.darkTheme` 是个 **facet**（控制它内部 `&dark` 基础规则：光标、选区、gutter 背景、tooltip/panel 背景），facet 只能用 Compartment 翻。于是 `darkSlot` 这个 Compartment 与 `data-theme` **必须同步更新**：store 写 `data-theme`、并经注入的 `applyDark` 回调调 workspace 的 `setDarkTheme` 翻 CM6 facet。⚠️ **漏掉任一条都会「半套主题」**：只翻 CSS 变量的话 CM6 的光标/选区/gutter 还停在旧色；只翻 facet 的话正文颜色不动。
> 4. 🔴 **`darkSlot` 是「共享」Compartment（全局视图设置），与 `lineWrapSlot` 同一类、与 `languageSlot`（每标签）相反。** 一次 reconfigure 翻**所有**标签的深浅色——主题是全局的，不该「只改当前看得见的那个标签，切过去才发现另一个还是旧色」。落地分两条路：① **显示中的分屏**走 `view.dispatch({ effects })`；② **没被任何分屏显示的标签**走 `applyViewConfig(tab, config)`（`state.update({ effects }).state` 存回 snapshot）。`setDarkTheme` 里两条都跑，与 `toggleLineWrap` 逐字同一套骨架。⚠️ `applyViewConfig` 的早退条件从「换行一致就退」改成「**换行与深浅色全都一致**才退」——只看一个的话，另一个不一致的标签会被漏掉（`tab.test.ts` 专钉了「只有深浅色不一致也要换 state」这条）。
> 5. 🔴 **`darkSlot` 做成 createEditorState 的必填参数（与 `lineWrapSlot` 对齐），不是可选。** 代价是 ~8 处测试调用点要补 `darkSlot: new Compartment()`；换来的是**单一代码路径 + 强不变量**——不存在「有的 state 有 darkSlot、有的没有」这种要处处判空的状态。`createEditorState` 仍收一个可选 `dark?: boolean`（默认 `true`）决定**初始**深浅色，`setup.test.ts` 钉了「传 `dark:false` 建出来一开始就是亮色」与「调用方的 darkSlot 能在没有 view 的 state 上 reconfigure」。
> 6. 🔴 **`theme.ts` 刻意不依赖 workspace / CM6；matchMedia 订阅的生命周期全在 store。** `theme.ts` 是纯注册表 + 纯函数（`resolveTheme` / `isDarkTheme` / `applyThemeAttr` 只动 `document.documentElement.dataset` / `watchSystemTheme` 只碰 `window.matchMedia`）。CM6 reconfigure 经 store 的**注入回调** `applyDark`（`App.tsx` 里接 `ws.setDarkTheme`）触发，store 自己不 import workspace。这样 `theme.ts` 能在 jsdom 里单测（`theme.test.ts` 12 条），CM6 那一半留给 workspace/store 的接线测试。
> 7. 🔴 **「跟随系统」的订阅是幂等的、且只在 `theme==='system'` 时挂着。** `syncSystemWatch()` 用 `unsubscribeSystem === null` 做闸：选 `system` 才订阅、选死亮/暗就退订（`applyNow` 与 `setTheme` 都会调它，重复调不会重复订阅）。**退订是安全语义不是省事**：用户明确选了亮/暗之后，OS 切换深浅色**不该**盖掉用户的选择——所以选死档时那个 matchMedia 监听必须是摘掉的。系统主题变化的回调只动 DOM 与 CM6、⛔ **不写穿配置**（这不是用户改动，盘上的 `theme` 仍是 `"system"`）。⚠️ jsdom 不实 `matchMedia`（`matches` 恒 false、无 change 事件），`theme.test.ts` 用 `installMatchMedia` 手搓一个能 `fire()` 的假 MQL 钉「系统偏好变化时回调解析出的具体主题，退订后不再回调」。
> 8. 🔴 **`theme` 在线上是不透明 `String`，sanitize 只在前端 store 夹一次。** 与 `fontSize` / 字体 ID 同一分工（M4-A 修正 4）：合法 ID 清单（`THEME_IDS = ['light','dark','system']`）是 **UI 概念**，Rust 侧 `Settings.theme` 是裸 `String`、不校验。前端 `sanitizeThemeId` 用 `THEME_IDS.includes(...)` 夹（数组 `includes` 不碰原型链，与字体 ID 用 `hasOwnProperty` 是同一类不可信输入防护），不认识的值退回 `DEFAULT_THEME`。`store.test.ts` 钉了「load 到非法 theme 退回默认」。
> 9. ⚠️ **安全姿势一个字节没动。** `save_settings` 仍**不收任何路径参数**（落点 `~/.vela/settings.json` 由 Rust `home_dir(app)` 算）；本轮**没有新增任何命令**（`theme` 只是既有 `load_settings` / `save_settings` 多带一个键）。`commands.rs` 那张「谁能碰到磁盘」的表仍 **17 行**，**Tauri command 仍 23**，managed state 仍 **4**，事件名仍 **8**，路径接受面没变宽。新增依赖 **0**（matchMedia 是浏览器 API，Compartment 早在 `@codemirror/state` 里）。⛔ **阶段2 的 `~/.vela/themes/` 自定义加载会第一次让「主题文件路径」进入信任面**——那一轮必须沿用同一条姿势（落点由 Rust 算、前端不传路径），现在只是把格式（JSON token 映射）定死、没碰磁盘。
> 10. **数字**：前端 **2697 → 2723（+26）**：新增 `settings/theme.test.ts` **12**、`settings/store.test.ts` 33 → **40**（+7，「主题（M4-C）」那一组）、`settings/AppearancePopover.test.tsx` 13 → **15**（+2，主题读数 + 主题 mutator）、`doc/tab.test.ts` **+3**（darkSlot 落到 state / 换行+深浅色同时 / 只有深浅色不一致也换 state）、`editor/setup.test.ts` **+2**（darkSlot 可在无 view 的 state 上 reconfigure / `dark:false` 初始亮色）。Rust **430 → 432（+2）**：vela-core lib 344 → **346**（+2，「theme 能存回来」+「项目层写 theme 也被忽略并记账」）、`wire_contract.rs` **49 不变**（黄金值加 `,"theme":"dark"`、条数没变）。🔴 **Tauri command 仍 23**，managed state 仍 4，事件名仍 8，路径接受面仍 17 行。新增依赖 **0**。**八道门禁全绿**，`stderr` 干净。<br>首屏 gzip 279.08KB → **279.78KB**（`index-DcC0Ltq2.js` **175.05** + `dist-uhAGXbQS.js` 100.21（逐字不变）+ `index-Ck8q8qUq.css` **4.52**），预算 300KB，余量 6.97% → **6.74%**（**+0.70KB**：entry +0.60 是 `theme.ts` + store 的 theme 键与 matchMedia 订阅 + 浮层多一个 select + `App.tsx` 的 `applyDark` 接线；CSS +0.10 是把颜色 token 从单块 `:root` 拆成「暗色默认 + 亮色 `[data-theme='light']`」两块并各带一条 `color-scheme`，哈希 `D1UC1zWI` → `Ck8q8qUq`）。⚠️ **modulepreload chunk 一个字节没动**（`dist-uhAGXbQS.js` 100.21，哈希逐字相同）——M4-C 没引入任何新懒加载边界，主题逻辑全是同步 import 进 entry 的（它太小，懒加载省不下什么）。
> 11. 🔴 **真机上要看的（jsdom 钉不住，一个数都没有）**：① 切亮/暗时**颜色与 CM6 facet 是不是真的同步翻**——jsdom 没有布局，`data-theme` 写没写、`darkTheme` facet 翻没翻都能验，但「正文/代码区/光标/选区/gutter/tooltip 是不是**一致地**换了色」只能在真 WKWebView 里看（第 3 条那两条路漏一条就是半套主题）；② **跟随系统**在真 macOS 上切深浅色时是不是实时跟上、且用户选死亮/暗后 OS 再切**不该**动它（第 7 条那条退订语义）；③ 多个标签/分屏时切主题是不是**全部**一起翻（第 4 条那条共享 Compartment，尤其「没被显示的那个标签」切过去后是不是新色）；④ 重启后 `theme` 是不是真的从盘上回来（写穿与 M4-A/B 同一条 promise 队列，但 `system` 档重启后要重新订阅 matchMedia 并解析成当前系统色）。

> ---
>
> **M4-C-2 实施修正**（主题系统 v1 阶段2：三套流行配色移植，2026-09-22）
>
> 1. 🔴 **License 逐个确认过，只选 MIT 的。** 调研了 6 套流行暗色主题：Dracula（MIT）、One Dark（MIT）、Nord（MIT）、Solarized（MIT）、Gruvbox（MIT/X11）、Monokai（商业授权，❌ 排除）。最终选了 **Dracula / Nord / Solarized** 三套，理由：① 全部 MIT License，闭源商业化无障碍；② 视觉风格差异明显——Dracula 紫调赛博朋克风、Nord 冷灰北欧极简、Solarized 暖黄低对比护眼；③ 每套都有完整的 11 个颜色 token 可映射（bg/bg-alt/bg-panel/fg/fg-dim/border/guide/accent/ok/warn/bad）。
> 2. 🔴 **resolveTheme 把三者都映射成 `'dark'`，因为全是暗色主题。** ThemeId 从 `['light', 'dark', 'system']` 扩到 `[..., 'dracula', 'nord', 'solarized']`，THEME_LABELS 加了三个中文标签（'Dracula' / 'Nord' / 'Solarized'，不翻译品牌名）。resolveTheme 里对这三者走 `return 'dark'`，isDarkTheme 自然返回 true，CM6 darkTheme facet 保持开启。⚠️ 这不是偷懒——如果将来要做一套「亮色 Dracula」或「亮色 Solarized」，那会是新的 ThemeId（如 `'dracula-light'`），而不是复用现有的 ID。
> 3. 🔴 **CSS 里加了三块 `:root[data-theme='*']` token 集，每块约 0.08KB。** Dracula 用 `#282a36` 作 bg、`#bd93f9` 作 accent、`#50fa7b` 作 ok；Nord 用 `#2e3440` 作 bg、`#81a1c1` 作 accent、`#a3be8c` 作 ok；Solarized 用 `#002b36` 作 bg、`#268bd2` 作 accent、`#859900` 作 ok。每套的 fg-dim/guide/border 都是从原配色的 selection/current line/comment 等语义色里挑最接近的。三块合计 +0.25KB CSS。
> 4. ⚠️ **安全姿势一个字节没动。** 仍是纯 CSS token 映射，没有让自定义主题注入任意 CSS 的能力（那是阶段2 推迟的那部分）。`~/.vela/themes/` 目录还没建，本轮只是把格式（JSON token 映射）定死、没碰磁盘。新增依赖 **0**。
> 5. **数字**：前端 **2723 → 2723（+0）**：`settings/theme.test.ts` **12 不变**（dracula/nord/solarized 的测试合并进现有的两个 it() 块，没有新建独立用例）、`settings/AppearancePopover.test.tsx` **15 不变**（只更新了主题 select 选项的预期值从 3 扩到 6）。Rust **432 不变**（theme 仍是那个 String 键，ID 清单在前端）。🔴 **Tauri command 仍 23**，managed state 仍 4，事件名仍 8，路径接受面仍 17 行。新增依赖 **0**。**八道门禁全绿**，`stderr` 干净。<br>首屏 gzip 279.78KB → **280.08KB**（`index-2CsICmfi.js` **175.10** + `dist-uhAGXbQS.js` 100.21（逐字不变）+ `index-CsjP9OTc.css` **4.77**），预算 300KB，余量 6.74% → **6.64%**（**+0.30KB**：entry +0.05 是 theme.ts 里新增的三个 ID 与映射逻辑；CSS +0.25 是三套主题的 token 块各约 0.08KB）。⚠️ **modulepreload chunk 一个字节没动**（`dist-uhAGXbQS.js` 100.21，哈希逐字相同）——M4-C-2 没引入任何新懒加载边界，主题 ID 与映射全是同步 import 进 entry 的。
