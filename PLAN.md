...
| **文档版本** | v1.1 |
| **日期** | 2026-09-14 |
| **状态** | ✅ **M0 闸门通过、无阻塞项 —— M1 可开工**。6 项验收通过（#2 #3 #5 #6 #7 #8）、#4 机制通过但首屏字体超预算 11%（转 M1 工作项）、#1 客观半通过（主观手感待人工滚动）。实测数据见 [`M0-REPORT.md`](M0-REPORT.md)，§3.2 是就地批注版 |
...
8. ✅ **M0 风险闸门已通过（2026-09-14）**：WKWebView 的滚动/输入手感这个生态级 open issue（tauri-apps/discussions#8436）**没有被证实**——机器空闲时帧计时贴着 60Hz vsync 上限（59.5~60.3fps，p95 ≤23ms），1 万行涨到 5 万行无可测退化；中文 IME 人工判定通过。**Tauri 2 路线确认，M1 可开工**。唯一剩下的未知是 #1 的主观滚动手感（合成滚动绕开了原生触控板惯性路径，自动化不了）。数据见 [`M0-REPORT.md`](M0-REPORT.md)。
...
**结论：🟢 继续按 Tauri 2 路线走，M0 闸门通过、无阻塞项 —— M1 可以开工** —— **6 项通过**（#2 #3 #5 #6 #7 #8）、**#4 机制 ✅ 但数值 ❌（首屏 2.219MB，超 2MiB 预算 11%）**、#1 客观半通过、主观半待人工滚动。上表的「当前状态」列与本节是同一批数据的就地批注版，**引用结论时以报告为准**。
...
> ⚠️ **#1 的主观半是 M0 剩下的最后一个未知**，也是唯一可能推翻 Tauri 路线的情形（判据：人工判定「手感不可接受」**且** Safari 对照组在同等负载下复现顺滑差异）。
...
**剩余工作**（截至 2026-09-14，#2 #3 #4 #5 #6 #7 #8 已结项，只剩 #1 的主观半）：
...
- **#1 只有一半可自动化**：合成滚动 + 帧计时能给回归基线，但绕开了触控板惯性经原生手势的那一段，答不了「手感」。仍需人滚一次；另欠安静机器复跑与 dPR=2 复测。
...
**测量方法论（十四条踩出来的教训，比数据本身更值钱；第 9~14 条是补齐 #4 字节口径那一轮新踩的）**：
