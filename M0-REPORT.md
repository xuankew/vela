...
**🟢 继续按 Tauri 2 路线走。M0 闸门通过，无阻塞项 —— M1 可以开工。**

八项验收：**7 项通过**（#1 #2 #3 #5 #6 #7 #8）、**1 项机制通过但数值超预算**（#4：首屏字体 **2.219MB** / 预算 2MB，超 11%，已转 M1 工作项）。**没有未决项，没有可能推翻路线的未知。**

支撑「继续」这个判断的核心事实有三条：

1. **R1（WKWebView 滚动手感）这个 🔴 高风险没有被证实，而且是被两条独立证据排除的。** 客观：机器空闲时帧计时贴着 60Hz 垂直同步上限（59.5~60.3fps，卡顿帧 0~2/179，p95 ≤23ms），且**1 万行涨到 5 万行没有可测量的退化** —— CM6 的视口虚拟化在我们的真实文档上成立。主观：用户在 5 万行文档上用触控板连续滚动，判定「没有卡顿，挺流畅」——**这条走的是合成滚动绕不开的原生手势路径，正是 discussions#8436 报微延迟的那一段**。
2. **后来观测到的 60→55fps 掉帧不是代码缺陷**，两次归因（冷字体分片、装饰插件每帧重建）都被受控实验推翻，特征符合外部 CPU 抢占。详见 §3。
3. **性能与体积预算四项达标且余量充足**：`.app` 23MB（余量 42%）、冷启动 635ms（余量 36%）、空转内存均值 104MB（余量 46%）、前端 bundle **219.58KB**（余量 27%，M0 探针脚手架已于 2026-09-14 删除，从 236.92KB 回落 17.34KB）。**唯一的硬超标是 #4 的首屏字体字节数 2.219MB（预算 2MB）** —— 根因是分片按码位区块切、一屏汉字散落到 25~36 片上，M1 按字频重排分片即可回到预算内（粗估 1.0~1.2MB），不动摇路线。

📌 **「有条件」的条件已经解除。** 原先唯一阻塞 M1 的验收项是 #2 中文 IME（结构性不可自动化，只有人能测），**用户已于 2026-09-14 人工判定通过**；同日 21:54 用户又人工判定 #1 的主观滚动手感通过 → **八项验收全部有结论，没有阻塞项，M1 可以开工**。§5 剩下的未闭合项只有 dPR=2（Retina）复测与安静机器帧率基线两条，**都不是决策输入**（前者是已知偏差的补测，后者只是给 M1 留一条干净的回归基线）；#4 超预算与长会话内存曲线已登记为 M1 工作项。
⚠️ **但有一个不属于验收项的独立阻塞**：`open` / LaunchServices 路径卡死（§5「启动」行）。它**不阻塞 M1 开发**（内层二进制可正常启动，本轮 #2 就是这么测的），**但阻塞正式分发**——Finder / Dock 双击才是用户的真实路径，需用户决定是否上真 Developer ID 签名 + 公证。

**✅ 「唯一会推翻路线的情形」已正式排除。** 原计划的推翻条件是两条同时成立：人工滚动判定「手感不可接受」**且** Safari 对照组在同等负载下复现出顺滑差异。**前半条现为「可接受」→ 整个条件不可能成立，Safari 对照组不必做**（客观半也已贴在 vsync 上限，没有差值需要解释）。至此 **M0 不存在任何可能推翻 Tauri 路线的未知**；剩下的全是补测与工作项，不是风险。
...
**已知偏差**：本报告全部数据取自 **dPR=1**。图形背板成本随 dPR 平方增长，Retina（dPR=2）下 #1 滚动与 #7 内存都会被**低估**，#2 的候选框定位还涉及坐标换算。📌 这**不使已做出的判定失效**（dPR=1 就是本轮的真实被测环境，判定在该环境下成立），但意味着 Retina 下的余量未知，**正式发布前需复测**。
...
| # | 验收项 | 判定 | 关键读数 |
|---|---|---|---|
| 1 | WKWebView 滚动手感 | ✅ **通过**（客观半 + 主观半） | 客观：低负载 59.5~60.3fps，p95 ≤23ms，卡顿 0~2/179。主观：用户 2026-09-14 21:54 在 5 万行文档上触控板连续滚动判定「没有卡顿，挺流畅」——**判定于 load 3.49，是更严苛条件下的通过** |
| 2 | 中文 IME | ✅ **通过**（人工判定） | 用户 2026-09-14 在真实窗口验证：无行跳动、候选框不错位、长句不丢字。⚠️ 只验了一种输入法、dPR=1 |
...
### #1 WKWebView 滚动手感 —— ✅ 通过（客观半 + 主观半均通过）

**判据**：万行文件滚动无肉眼卡顿，主观手感可接受。
**为什么 60fps 是上限而不是巧合**：外接屏 @60.00Hz，垂直同步上限就是 60fps。

#### 主观半 —— ✅ 通过（人工判定，2026-09-14 21:54）

用户在真实 WKWebView 窗口里按指引切到 **5 万行**文档、用**触控板**连续滚动（含惯性甩动与突然反向），判定：**「没有卡顿，挺流畅的」**。四条判据（无肉眼卡顿、主观手感可接受）均满足。被测产物是 `…/Vela.app/Contents/MacOS/vela`（release，与 #2 / #4 同一份构建），dPR=1。

📌 **这条判定是在 `load averages 3.49 / 3.03 / 2.98` 下做出的，而这是更强的证据，不是更弱的。** 判定规则是不对称的：负载高时若手感好，说明在比目标环境更严苛的条件下已经达标（**下界**）；负载高时若手感差，才不能归因于 Tauri。本轮落在前一种。

📌 **它闭合了 M0 最大的那个未知。** 原计划里「唯一会推翻 Tauri 路线的情形」是：人工判定手感不可接受 **且** Safari 对照组复现顺滑差异。前半条现在明确为「可接受」→ **该情形正式排除，Safari 对照组不必做**（客观半也已贴在 vsync 上限，没有差值需要解释）。这条正是合成滚动答不了的那一半：程序化写 `scrollTop` 绕开了触控板惯性经 WKWebView 原生手势的路径，而 discussions#8436 报告的微延迟恰好在那条路径上——**现在那条路径被人滚过了**。

⚠️ **诚实标注：这是 M0 唯一没有数据产物的验收项。** 人工判定不经过探针、不落盘，所以没有 `.json` 可复核，记录只有「谁、什么时候、什么负载、说了什么」。同时它是**单次判定**，没有像 #3 那样多轮复现；「5 万行 / 30 秒 / 触控板」是按我给的指引执行的，没有独立埋点验证实际执行了多少。这不改变结论（主观手感本来就只能这么测），但引用时应记住它的证据强度低于同报告里的量化项。
⚠️ dPR=1，Retina 下每帧要画的像素是 4 倍，需与 #7 合并复测。
...
**客观半自己的边界与剩余补测**：
- ✅ **合成滚动答不了的那一半已由人滚补上**（见上面的主观半）。边界认知保留：两档客观数据都是**合成滚动**（程序化写 `scrollTop`），绕开了触控板惯性经 WKWebView 原生手势的那一段，而 `tauri-apps/discussions#8436` 报告的微延迟恰好在**那条路径**上 → 帧计时永远只能给回归基线，给不了「手感」。
- 📌 **安静机器（`load < 1`）复跑已降级**，不再是决策输入：它原本是为了闭合「60→55fps 是否代码退化」，而噪声带实验加高负载下的主观通过已经闭合了那个疑问，现在只剩「给 M1 留一条干净的帧率回归基线」的价值。
- ⚠️ dPR=2（Retina）复测（发布前必须）；20k 档未单列（被 10k/50k 夹逼）。

📌 原计划的 **Safari 对照组确定不必做**，两条独立理由：① 低负载客观读数已贴在 60Hz 上限，没有「Tauri 比 Safari 差」的差值需要解释；② 推翻条件是「手感不可接受 **且** Safari 复现顺滑差异」，前半条已被否定，合取式不可能成立。
...
**判定**：✅ **通过**。用户于 2026-09-14 21:35 左右在真实 WKWebView 窗口里人工输入验证，四条判据均无异常。被测产物是 `…/Vela.app/Contents/MacOS/vela`（release 构建，与 #4 同一份），dPR=1。

📌 **这是 M0 唯一一项结构性不可自动化的验收项，它通过意味着闸门不再有阻塞项。**

**为什么这一项不可自动化**（不是偷懒，是路径问题）：
- 程序化插入文本会**绕过 `compositionstart/update/end`** —— 而那三个事件正是被测路径本身。
- 合成键盘事件（`KeyboardEvent`）**驱动不了 IME 候选窗**，候选窗由系统输入法进程绘制，不在 WebView 的事件模型里。

**已备好的工具**：探针面板里有**原生 `<textarea>` 对照组**，用来区分「是 CM6 的问题」还是「是 WKWebView 的问题」—— 这个区分直接决定对策。两边都正常时该区分不产生信息，所以本轮判定不依赖它。

⚠️ **两条未覆盖面（都不阻塞 M1，但正式发布前要补）**：
1. **只验了一种输入法。** 不同输入法的 composition 事件序列差别很大（系统拼音 / 双拼 / 五笔 / 仓颉，以及搜狗、微信输入法这类第三方输入法会自己绘候选窗），`inputStyle` 的问题往往只在某一种上暴露。M1 做编辑器内核时应至少在系统拼音 + 一款第三方输入法上各过一遍。
2. **dPR=1。** 候选框定位涉及屏幕坐标换算，Retina（dPR=2）下需复测——与 #1 / #7 的 dPR=2 复测合并做。

**若将来复现问题的对策**：调整 CM6 `inputStyle`（`contenteditable` vs `textarea`）；参考 Monaco #4592 的教训。
...
构建确实踩中三个坑：**rolldown 的 `manualChunks` 形式**、**esbuild 不再内置**、**粗分包摧毁懒加载**。首屏 gzip 一度冲到 **623KB**，修复后 **284.4KB / 114 chunk**（脚手架删除后当前 **219.58KB / 117 chunk**）。详见 `PLAN.md` §2.3。
...
| 前端 bundle（gzip） | ≤ 300KB | ✅ **219.58KB**（脚手架删除后复测） | 27% |
...
**首屏 gzip 219.58KB 的构成**（2026-09-14 删除 M0 脚手架后复测；口径不含 `index.html` 的 0.29KB）：

| 文件 | min | gzip |
|---|---|---|
| `index-*.js`（入口：Solid + 应用 + CM6 基础扩展） | 302.83K | **110.23 KB** |
| `dist-*.js`（CM6 内核，modulepreload） | 335.25K | **108.50 KB** |
| `index-*.css`（应用自身样式，字体声明已移出） | 2.29K | **0.85 KB** |
| **首屏合计** | | **219.58 KB** |
...
> ✅ **已兑现：M0 探针脚手架整体删除，首屏 gzip 236.92 → 219.58KB**（-17.34KB）。被压进入口 chunk 的两个静态 import 是 `src/probe/sweep.ts`(17.5KB) 与 `ProbePanel.tsx`(46.9KB)。比 D7 基线 218.5KB 多出的 1.08KB 是清理后 `App.tsx` 的工具栏与字体切换 UI，不是残留脚手架。
> **别把当初那 18.4KB 记成 D2 分字体的代价** —— Maple 的 CSS 走 `?inline` 动态 import，独立成 `regular-*.js`，首屏一个字节都没碰。

---

## 5. 未闭合项清单

📌 **八项验收全部有结论，无阻塞项**（#2 与 #1 主观半分别于 2026-09-14 人工判定通过后解除）。下表「阻塞」一列现在只剩两类：`发布` = 不影响 M1 开发但影响正式分发；`M1` = 转成 M1 的工作项。**表里已没有任何「可能推翻路线」的未知。**

| # | 待补 | 谁能做 | 阻塞？ |
|---|---|---|---|
| #2 | ✅ **已通过**：用户 2026-09-14 在真实 WKWebView 窗口人工输入判定，四条判据（行跳动 / 候选框错位 / 长句丢字 / 光标位置）均无异常 | 已完成 | —— |
| #2 | ⚠️ **只验了一种输入法、dPR=1**：不同输入法的 composition 事件序列差别很大（第三方输入法自己绘候选窗），Retina 下候选框定位涉及坐标换算 | 用户 + 我可做 | M1（内核阶段顺带过一遍） |
| #1 | ✅ **已通过**：用户 2026-09-14 21:54 在 5 万行文档上用触控板连续滚动，判定「没有卡顿，挺流畅」。判定于 `load 3.49 / 3.03 / 2.98`——按不对称证据规则这是**下界**（更严苛条件下达标），比安静机器上的通过更有说服力。⚠️ 这是 M0 唯一没有数据产物的验收项（人工判定不落盘、单次、无埋点核实实际滚了多少行），证据强度低于同报告的量化项 | 已完成 | —— |
| #1 | **安静机器复跑**：`load < 1` 时跑同一个构建，确认回到 ~60fps。📌 **本项已降级，不再是决策输入**——它原本是为了闭合「60→55fps 是不是代码退化」这个疑问，而噪声带实验（同一构建三次连跑 53.2~57.1fps，带宽 3.9fps）已把两次归因推翻、本次高负载下的主观通过又给了更强的证据，**那个疑问已经闭合**。剩下的价值只是给 M1 留一条干净的帧率回归基线 | 需机器空闲（我不能杀用户的 VM / Qoder / Codex；实测最大占用者常是 Qoder 自己的 Renderer） | 否 —— 2026-09-14 21:33 尝试时 `load 3.23`、WindowServer 24%，已推迟；可在 M1 开工后顺带补 |
| #1 #7 #2 | **dPR=2（Retina）复测**：滚动帧率、可见态内存、IME 候选框定位 | 需接 Retina 屏 | 否，但正式发布前必须 |
| #4 | ✅ **已补齐**：清单查表口径落地（433 片 / 18.07MB），首屏实测 **2.219MB**，全部样本 `trustworthy` | 已完成 | —— |
| #4 | **首屏字体超预算 11%**：按字频重排分片，把常用 ~3500 字集中到头 1~2 片（粗估可降到 1.0~1.2MB）。⛔ 不要改用精简子集，那会触发 OFL 改名义务 | 我可做，已登记为 PLAN.md §3.3 的 M1 工作项（0.5 人日） | M1 |
| 启动 | ⛔ **`open` / LaunchServices 路径卡死**：同一个 `.app`，直接跑内层二进制一切正常，`open` 停在 `page-load Started` 不再前进、前端 IPC 一条不发。已排除：report_path、命令注册、旧 bundle、开关文件位置、`panelOpen`、`autotest` prop、hidden 页面字体加载、osascript 窗口读数、系统休眠（caffeinate 生效、assertions=1）、bundle 签名（现在 `valid on disk` / `satisfies its Designated Requirement` / `Identifier=app.vela.m0`）、hardened runtime、`com.apple.provenance` xattr、重复注册、translocation。`spctl -a -t exec` 仍报 `rejected`（ad-hoc、无 Developer ID、未公证） | **需用户决定**是否上真 Developer ID 签名 + 公证 | **发布：是**（Finder / Dock 双击才是用户的真实路径）<br>**M1 开发：否**（内层二进制可正常启动，本轮 #2 人工判定就是这么测的） |
...
| #7 | **长会话内存曲线**：反复开关大文件是否阶梯式上涨、内存压力下是否回收 | 我可做 | M1 |
...
📌 **`sweep.ts` 的去留已由用户拍板：删。** 它结案后只服务两件**不阻塞 M1** 的补测——dPR=2（Retina）复测（#1/#7/#2，发布前必须）与安静机器帧率基线（已降级为回归基线）。留着 = 首屏多 18.4KB 直到复测做完；删掉 = 首屏立刻回落，但复测时要重写。**已随 `src/probe/` 整体删除，Retina 复测排到发布前时从 git 历史取回或重写。**
...
1. **仓库根的 `.m0-*` 产物（72 个）、`..m0-shot.png-TVuZ`、`.build-log.txt`、`~/Library/Logs/vela-m0-boot.log` 都没有被 git 跟踪**（前者被 `.gitignore` 忽略，后三者从未 `add`），硬删不可恢复，所以是**移到了 `~/.Trash/vela-m0-cleanup-20260914-225627/`（5.5MB）而不是 `rm`**。`scripts/font-manifest.mjs` 是**已跟踪**的，走 `git rm` 等价路径（工作区删除，可从历史取回）。要彻底腾空间就清空回收站。
2. **清理后的 `App.tsx` 是重写的，只做了静态核验，没有真正跑起来看过一眼** —— 浏览器导航被环境拦截、`screencapture` 会挂死、dev 构建没开 `devtools`，三条观测通道都不可用。**下次启动 Vela 时顺手确认一下工具栏与编辑器正常**（空文档挂载、打开文件、两个字体下拉、字号、换行开关）。

---

## 7. 方法论沉淀

十五条踩出来的教训已就地写在 `PLAN.md` §3.2「测量方法论」，比本报告的任何数据都更值钱（第 9~14 条是补齐 #4 字节口径这一轮新踩的，第 15 条是 #1 主观判定这一轮）。摘要：
...
15. **主观性能判定的证据规则是不对称的** —— 高负载下判「流畅」是**下界**（比目标环境更严苛的条件下已达标，结论更强）；高负载下判「卡」才不能归因于 Tauri（需复测）。所以「等一台安静机器再测 #1 主观半」是不必要的挂起：两个分支都能得出结论时，当前条件就够用。这条与第 7 条（差值小于噪声带不能报结论）成对——第 7 条管「不能说」，本条管「其实已经能说」。**同时人工判定要如实标注证据强度**（不落盘、单次、无埋点核实实际滚动量）：把弱证据写得和强证据一样确定，是另一种失真。
