...
📌 **`sweep.ts` 的去留是个待决权衡，不要默默删**：#1 结案后它只服务两件**不阻塞 M1** 的补测——dPR=2（Retina）复测（#1/#7/#2，发布前必须）与安静机器帧率基线（已降级为回归基线）。留着 = 首屏多 18.4KB 直到复测做完；删掉 = 首屏立刻回落到 ~218KB，但复测时要重写。**建议顺序：M1 开工前先删（首屏干净、开发期不背脚手架），Retina 复测排到发布前再从 git 历史取回或重写**——但这是用户的决定。
