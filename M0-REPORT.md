...
八项验收：**6 项通过**（#2 #3 #5 #6 #7 #8）、**1 项机制通过但数值超预算**（#4：首屏字体 **2.219MB** / 预算 2MB，超 11%）、**1 项客观半通过、主观半待人工滚动**（#1）。
...
📌 **「有条件」的条件已经解除。** 原先唯一阻塞 M1 的验收项是 #2 中文 IME（结构性不可自动化，只有人能测），**用户已于 2026-09-14 人工判定通过** → **八项验收现在没有阻塞项，M1 可以开工**。§5 剩下的未闭合项里，#1 主观手感 / 安静机器复跑 / dPR=2 复测都不阻塞，#4 超预算与长会话内存曲线已登记为 M1 工作项。
...
**唯一会推翻路线的情形**：人工滚动判定「手感不可接受」，**且** Safari 对照组在同等负载下复现出顺滑差异。⚠️ **前半条仍未测**——客观半已通过，但合成滚动绕开了触控板惯性经原生手势的那一段，而 discussions#8436 报的微延迟恰好在那条路径上。这是 M0 剩下的最后一个未知。
...
| # | 验收项 | 判定 | 关键读数 |
|---|---|---|---|
| 1 | WKWebView 滚动手感 | 🟡 客观半 ✅ / 主观半待人工滚动 | 低负载 59.5~60.3fps，p95 ≤23ms，卡顿 0~2/179 |
...
📌 **八项验收已无阻塞项**（#2 于 2026-09-14 人工判定通过后解除）。下表「阻塞」一列现在只剩两类：`发布` = 不影响 M1 开发但影响正式分发；`M1` = 转成 M1 的工作项。
...
| #1 | **人工滚动手感**：5 万行文档连续滚 30s，判断有无肉眼卡顿 | **只有用户**（合成滚动绕开了原生手势路径） | 否（客观半已通过）⚠️ 但这是**唯一可能推翻路线**的未知 |
| #1 | **安静机器复跑**：`load < 1` 时跑同一个构建，确认回到 ~60fps | 需机器空闲（我不能杀用户的 VM / Qoder / Codex；实测最大占用者常是 Qoder 自己的 Renderer） | 否 —— 2026-09-14 21:33 尝试时 `load 3.23`、WindowServer 24%，已推迟 |
