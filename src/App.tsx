  ...
  return (
    <div class="app">
      <div class="toolbar">
        ...
        {/* 形状照抄上面「换行」那一组：一个标签 + 一个开/关按钮。
            不 `disabled`：非 Markdown 文档按下去面板照样出来，里面写着
            「TypeScript 还没有符号表」——那比按钮灰着说明得多，理由同 builtins.ts 里那条。

            排在「预览」**前面**，与屏幕上两栏的左右顺序一致（大纲在左、预览在右）：
            工具栏上挨着的两个开关，点下去却在窗口两端各开一栏，那种对应关系要靠猜 */}
        <div class="toolbar-group">
          <span class="toolbar-label">大纲</span>
          <button onClick={() => void registry.execute('view.toggleOutline')} title="Mod+Shift+M">
            {outlineVisible() ? '开' : '关'}
          </button>
        </div>
        ...
        <div class="toolbar-group" style={{ 'border-right': 'none' }}>
          ...
          <button onClick={() => void registry.execute('view.togglePreview')} title="Mod+Shift+V">
            {previewVisible() ? '开' : '关'}
          </button>
        </div>
      </div>
